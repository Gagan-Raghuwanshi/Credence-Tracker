  ...
  try {
    ...
    for (let i = 0; i < historyData.length; i++) {
      ...
      if (i > 0 && item.attributes.totalDistance) {
        ...
        totalDistance +=
          ...
          (prevItem?.attributes?.totalDistance || 0);
      } else {
    ...
    if (previousType && startTime) {
      ...
      const duration = Math.floor(
        (new Date(lastItem.deviceTime) - new Date(startTime)) / 1000
      );
      ...
      typesOnly.push({
        ...
        consumption: null,
        initialFuelLevel:
          historyData[historyData.length - 2]?.attributes.fuel || null,
...
export const distanceReport = async (req, res) => {
    try {
      ...
      }).select("deviceTime attributes.distance deviceId");
      ...
      distanceData.forEach((item) => {
        const { deviceId, deviceTime, attributes } = item;
        const distance = attributes.distance / 1000;
  
        const deviceTimeString =
        ...
        const date = deviceTimeString.split("T")[0];
  
        let deviceEntry = result.find((d) => d.deviceId === deviceId);
        ...
        // Sum the distance for the specific date
        if (!deviceEntry[date]) {
...
export const vehiclelog = async (req, res) => {
  try {
    ...
    const attributesToSelect =
      attribute === "all" ? {} : { [attribute]: 1, added: 1 };
